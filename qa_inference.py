...
from feedbackQA.data.predict import Predictor, generate_embeddings, PredictorWithOutlierDetector
...
def main():
    ...
    parser.add_argument("--do_mtl", help="will log more info", action="store_true")
    ...
    print(hyper_params)
    if False:#hyper_params['use_parlai']:
    ...
    if args.train:
        ...
        if not args.use_original_model_parameters:
            model_ckpt = torch.load(ckpt_to_resume, map_location=torch.device("cpu"))
            ret_trainee.load_state_dict(model_ckpt["state_dict"])
            #ret_trainee.load_state_dict(model_ckpt)
        else:
            logger.warning("will NOT load the model parameters - just use the pre-trained model")
        ...
        source2pred, source2label, questions = predictor.generate_predictions(
        ...
        with open(args.predict_to, 'w+', encoding='utf-8') as f:
            for src, pred in source2pred.items():
                #with open(args.feedback_output_file, 'w+') as f:
                #    json.dump(source2pred, f)
                #with open(args.feedback_output_file+'.label', 'w+') as f:
                #    json.dump(source2label, f)
            ...
            with open(args.feedback_output_file, 'w+') as f:
                f.write('the accuracy of {0} is:\t{1}'.format(args.predict, acc))
...
def init_model(
    ...
    ModelTrainer = RetrieverTrainer
    ret_trainee = ModelTrainer(
        ret,
        train_dataloader,
        dev_dataloaders,
        test_dataloader,
