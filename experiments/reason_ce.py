...
from feedbackQA.data.predict_rate import generate_embeddings, PredictorWithOutlierDetector, Predictor
from feedbackQA.data.generation_dataloader import generate_dataloader
from feedbackQA.data.reason_dataloader import ReasonDataset
...
def main():
    ...
    parser.add_argument("--rate_wght", type=float, default=1.)
    parser.add_argument(
    ...
    parser.add_argument('--do_rl', action="store_true")
    parser.add_argument("--fb_for_rate", help="none", choices=['single', None, 'multiple'], default=None)
    parser.add_argument("--num_class", choices=['2', '4'], default='4')
    ...
    if 't5' in hyper_params['model']['bert_base']:
        args.rate_wght = 0.
...
def init_model(
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,
        verbose=True,
        monitor=("val_acc_avg" if args.rate_wght>0 else "val_loss_0"),
        mode="max" if args.rate_wght>0 else "min",
        period=0,
    )

    early_stopping = EarlyStopping(
        ("val_acc_avg" if args.rate_wght>0 else "val_loss_0"), 
        mode="max", patience=hyper_params["patience"]
    )
...
def get_data_loaders(hyper_params, num_workers, tokenizer, n_gpus, fb_for_rate):
