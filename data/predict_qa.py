...
class Predictor:
    ...
    def __init__(self, retriever_trainee):
        ...
        self.max_question_len = 50#self.retriever_trainee.retriever.max_question_len
        try:
            self.tokenizer = self.retriever_trainee.retriever.tokenizer
        except:
            self.tokenizer = self.retriever_trainee.model.tokenizer
        try:
            self.retriever = retriever_trainee.retriever
        except:
            self.retriever = retriever_trainee.model
    ...
    def generate_predictions(self, json_file, predict_to, multiple_thresholds, write_fix_report):
        ...
        relevance_scores_list, source2pred, questions, sources, normalized_scores, source2label, answers = res
        return source2pred, source2label#, answers
        #generate_and_log_results(relevance_scores_list, indices_of_correct_passage, normalized_scores, predict_to,
    ...
    def compute_results(self, json_data, passage_id2index, source2passages):
        ...
        for example in tqdm(json_data["examples"]):
            ...
            answers.append(source2passages[source][example["passage_id"]])
            index_of_correct_passage = passage_id2index[example["passage_id"]]

            relevance_scores, prediction, norm_score = self.make_single_prediction(question, source,
                                                                 source2embedded_passages)
            source2pred[source].append(prediction.item())
            relevance_scores_list.append(relevance_scores.numpy())
            source2label[source].append(index_of_correct_passage)
            normalized_scores.append(norm_score.item())
            indices_of_correct_passage.append(index_of_correct_passage)
            
        return relevance_scores_list, source2pred, questions, sources, normalized_scores, source2label, answers
...
def rerank_result_at_threshold(
    predictions, indices_of_correct_passage, normalized_scores, threshold, log_threshold
):
    count = len(indices_of_correct_passage)
    ood_count = sum([x == -1 for x in indices_of_correct_passage])
